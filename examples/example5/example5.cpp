...
void cpp_string(const char* msg) {
    std::cout << "Hello, " << msg << "! Welcome to my C++ World." << std::endl;
}
...
namespace Example2
{
    namespace Test
    {
        void foo()
        {
            std::cout << "Called Example2::Test::foo()" << std::endl;
        }
    } // namespace Test

    void foo()
    {
        std::cout << "Called Example2::foo()" << std::endl;
    }

    // class Foo2
    // {
    // public:
    //     Foo2() { std::cout << "Creating new Example2::Foo2" << std::endl; }

    //     Foo2(int32_t i) { m_foo = i; }

    //     ~Foo2() { std::cout << "Destroying Example2::Foo2" << std::endl; }
    ...
    //     void foo()
    //     {
    //         std::cout << "This Example2::Foo2 object has m_foo set to " << m_foo << " and m_bar set to " << m_bar
    //                   << std::endl;
    //     }
    //     void bar(int32_t a /*, char b*/) { std::cout << "Example2::Foo2::bar(): value of a: " << a << std::endl; }
